...
export default function ProductCategory() {
    ...
    const handleDelete = async (id) => {
        ...
        if (shouldDelete) {
            try {
                await axios.delete(`${process.env.REACT_APP_API_URL}/api/product_categories/delete/${id}`);
                fetchData();
    ...
    const handleSearch = async (term) => {
        ...
        const filtered = expenseCategories.filter((product_category) => {
            let nameMatch = product_category.name.toLowerCase().includes(term.toLowerCase());
    ...
    return (
        <Layout>
            ...
            <div className="flex justify-end mb-5">
                <NavLink to={'/products'} className="flex justify-center bg-[#4c5165] text-gray-300 font-bold px-5 rounded-md mr-5">
                    <span className="mr-1">Product List</span>
                    <ListAltOutlinedIcon />
                </NavLink>


                <NavLink to={'/product_categories/create'} className="flex justify-center bg-[#4c5165] text-gray-300 font-bold px-5 rounded-md">
                    New
                    <AddOutlinedIcon />
                </NavLink>
            </div>
            ...
            <table className="w-full text-xs text-left text-gray-500 shadow-md rounded-lg mt-2 mb-2" id="table-to-xls">
                ...
                <tbody className="[&>tr>td]:px-4 [&>tr>td]:py-1 [&>tr>td]:font-normal [&>tr>td]:text-gray-900 [&>tr>td]:text-center [&>tr>td]:border">
                    {displayData.map((product_category, index) => (
                        <tr key={product_category._id} className="bg-white border-b hover:bg-gray-100">
                            <td className=" w-[80px]">
                                {calculateReversedSL(startIndex + index)}
                            </td>
                            <td>
                                {capitalizeFirst(product_category.name)}
                            </td>
                            ...
                            <td className=" w-[100px]">
                                ...
                                <NavLink
                                    to={`/product_categories/update/${product_category.name}`}
                                    className="mr-3 [&>svg]:text-yellow-400"
                                >
                                    <EditOutlinedIcon />
                                </NavLink>

                                <button onClick={() => handleDelete(product_category._id)} className="[&>svg]:text-red-400"><DeleteOutlineOutlinedIcon /></button>
                            </td>
                        </tr>
                    ))}
                </tbody>
