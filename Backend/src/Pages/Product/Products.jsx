...
export default function Product() {
    ...
    const handleSearch = async (term) => {
        ...
        const searchFiltered = products.filter((product) => {
            let nameMatch = product.name.toLowerCase().includes(term.toLowerCase());
            let darazNameMatch = product.daraz_name.toLowerCase().includes(term.toLowerCase());
            let descriptionMatch = product.description.toLowerCase().includes(term.toLowerCase());
            let categoryMatch = product.category.name.toLowerCase().includes(term.toLowerCase());

            // let amountMatch = typeof product.amount === 'string' ? product.amount.includes(term) : product.amount.toString().includes(term);

            return nameMatch || darazNameMatch || descriptionMatch || categoryMatch;
        });
    ...
    return (
        <Layout>
            ...
            <div className="flex justify-end mb-5">
                <NavLink to={'/product_categories'} className="flex justify-center bg-[#4c5165] text-gray-300 font-bold px-5 rounded-md mr-5">
                    <span className="mr-1">Product Categories</span>
                    <ListAltOutlinedIcon />
                </NavLink>

                <NavLink to={'/products/create'} className="flex justify-center bg-[#4c5165] text-gray-300 font-bold px-5 rounded-md">
                    New
                    <AddOutlinedIcon />
                </NavLink>
            </div>
                ...
                <tbody className="[&>tr>td]:px-4 [&>tr>td]:py-1 [&>tr>td]:font-normal [&>tr>td]:text-gray-900 [&>tr>td]:text-center [&>tr>td]:border">
                    {displayData.map((product, index) => (
                        <tr key={product._id} className="bg-white border-b hover:bg-gray-100">
                            ...
                            <td>
                                {capitalizeFirst(product.category.name)}
                            </td>
                            <td>
                            ...
                            <td className=" w-[100px]">
                                ...
                                <button onClick={() => handleDelete(product._id)} className="[&>svg]:text-red-400"><DeleteOutlineOutlinedIcon /></button>
                            </td>
                        </tr>
                    ))}
                </tbody>
