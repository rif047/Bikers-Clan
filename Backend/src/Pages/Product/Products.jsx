...
export default function Expenses() {
    ...
    const handleDelete = async (row) => {
        if (window.confirm(`Are you sure you want to delete ${row.name.toUpperCase()}?`)) {
            try {
                await axios.delete(`${import.meta.env.VITE_API_URL}/${EndPoint}/${row._id}`);
                toast.success(`${row.name.toUpperCase()} has been deleted.`);
                fetchData();
            } catch (error) {
    ...
    const columns = [
        ...
        { accessorKey: 'buying', header: 'Buying', size: 80 },
        { accessorKey: 'selling', header: 'Selling', size: 80 },
        { accessorKey: 'stock', header: 'Stock', size: 80 },
        { accessorKey: 'feature', header: 'Feature', size: 80 },
        { accessorKey: 'onWeb', header: 'Website', size: 80 },
        {
            accessorKey: 'images',
            header: 'Images',
            size: 70,
            Cell: ({ cell }) => {
                ...
                return Array.isArray(images) && images.length > 0 ? (
                    <img
                        src={`http://localhost:9000/Images/${EndPoint}/${images[0]}`}
                        alt="Voucher"
        ...
        <Layout>
            ...
            <section className="flex justify-between px-5 py-2 mb-3 bg-[#4c5165]">
                <div className='flex justify-center items-center'>
                    <h1 className="font-bold text-sm md:text-lg text-white mr-2">Product List</h1>
                    {loading ? (
