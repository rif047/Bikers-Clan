...
export default function Supplier() {
    ...
    const handleDelete = async (id) => {
        ...
        if (shouldDelete) {
            try {
                await axios.delete(`${process.env.REACT_APP_API_URL}/api/suppliers/delete/${id}`);
                fetchData();
    ...
    const handleSearch = async (term) => {
        ...
        const filtered = suppliers.filter((supplier) => {
            let nameMatch = supplier.name.toLowerCase().includes(term.toLowerCase());
            let phoneMatch = supplier.phone.includes(term);
            let altPhoneMatch = supplier.alt_phone && supplier.alt_phone.includes(term);
            let addressMatch = supplier.address.toLowerCase().includes(term.toLowerCase());

            return nameMatch || phoneMatch || altPhoneMatch || addressMatch;
        });
    ...
    return (
        <Layout>
            ...
            <div className="flex justify-end mb-5">
                <NavLink to={'/suppliers/create'} className="flex justify-center bg-[#4c5165] text-gray-300 font-bold px-5 rounded-md">
                    New
                    <AddOutlinedIcon />
                </NavLink>
            </div>
            ...
            <table className="w-full text-xs text-left text-gray-500 shadow-md rounded-lg mt-2 mb-2" id="table-to-xls">
                ...
                <tbody className="[&>tr>td]:px-4 [&>tr>td]:py-1 [&>tr>td]:font-normal [&>tr>td]:text-gray-900 [&>tr>td]:text-center [&>tr>td]:border">
                    {displayedCustomers.map((supplier, index) => (
                        <tr key={supplier._id} className="bg-white border-b hover:bg-gray-100">
                            <td className=" w-[80px]">
                                {calculateReversedSL(startIndex + index)}
                            </td>
                            <td>
                                {capitalizeFirst(supplier.name)}
                            </td>
                            <td>
                                {supplier.phone}
                            </td>
                            <td>
                                {supplier.alt_phone === undefined || supplier.alt_phone === supplier.phone ? 'N/A' : supplier.alt_phone}
                            </td>
                            <td>
                                {supplier.address}
                            </td>
                            <td>
                                {supplier.description ? supplier.description : 'N/A'}
                            </td>
                            <td className=" w-[100px]">

                                <NavLink
                                    to={`/suppliers/update/${supplier.phone}`}
                                    className="mr-3 [&>svg]:text-yellow-400"
                                >
                                    <EditOutlinedIcon />
                                </NavLink>

                                <button onClick={() => handleDelete(supplier._id)} className="[&>svg]:text-red-400"><DeleteOutlineOutlinedIcon /></button>
                            </td>
                        </tr>
                    ))}
                </tbody>
