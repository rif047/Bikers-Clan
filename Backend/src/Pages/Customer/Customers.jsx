...
export default function Customer() {
    ...
    const reversedCustomers = [...filteredCustomers].reverse();
    const startIndex = currentPage * itemsPerPage;
    const endIndex = Math.min((currentPage + 1) * itemsPerPage, reversedCustomers.length);
    const displayedCustomers = reversedCustomers.slice(startIndex, endIndex);
    const totalPages = Math.ceil(reversedCustomers.length / itemsPerPage);
    const calculateReversedSL = (index) => reversedCustomers.length - index;
    ...
    const fetchData = async () => {
        try {
            ...
            const filteredRoleCustomers = allCustomers.filter(customer => customer.role === 0);
            setCustomers(filteredRoleCustomers);
            setFilteredCustomers(filteredRoleCustomers);
        } catch (error) {
            console.error(error);
        }
    };
    ...
    const handleDelete = async (id) => {
        ...
        if (shouldDelete) {
            try {
                await axios.delete(`${process.env.REACT_APP_API_URL}/api/customers/delete/${id}`);
                fetchData();
    ...
    const handleSearch = async (term) => {
        ...
        const filtered = customers.filter((customer) => {
            let nameMatch = customer.name.toLowerCase().includes(term.toLowerCase());
            let phoneMatch = customer.phone.includes(term);
            let altPhoneMatch = customer.alt_phone && customer.alt_phone.includes(term);
            let addressMatch = customer.address.toLowerCase().includes(term.toLowerCase());
            let shippingAddressMatch = customer.shipping_address.toLowerCase().includes(term.toLowerCase());
            let emailMatch = customer.email && customer.email.toLowerCase().includes(term.toLowerCase());
    ...
    return (
        <Layout>
            ...
            <div className="flex justify-end mb-5">
                <NavLink to={'/customers/create'} className="flex justify-center bg-[#4c5165] text-gray-300 font-bold px-5 rounded-md">
                    New
                    <AddOutlinedIcon />
                </NavLink>
            </div>
                ...
                <div className="flex mb-3 justify-center">
                    <div className="">
                        ...
                        >
                            ...
                            <option value={filteredCustomers.length}>All</option>
                        </select>
                    </div>
            ...
            <table className="w-full text-xs text-left text-gray-500 shadow-md rounded-lg mt-2 mb-2" id="table-to-xls">
                ...
                <tbody className="[&>tr>td]:px-4 [&>tr>td]:py-1 [&>tr>td]:font-normal [&>tr>td]:text-gray-900 [&>tr>td]:text-center [&>tr>td]:border">
                    {displayedCustomers.map((customer, index) => (
                        <tr key={customer._id} className="bg-white border-b hover:bg-gray-100">
                            <td className=" w-[80px]">
                                {calculateReversedSL(startIndex + index)}
                            </td>
                            <td>
                                {capitalizeFirst(customer.name)}
                            </td>
                            <td>
                                {customer.phone}
                            </td>
                            <td>
                                {customer.alt_phone === undefined || customer.alt_phone === customer.phone ? 'N/A' : customer.alt_phone}
                            </td>
                            <td>
                                {customer.address}
                            </td>
                            <td>
                                {customer.shipping_address === undefined || customer.shipping_address === customer.address ? 'N/A' : customer.shipping_address}

                            </td>
                            <td>
                                {customer.email === undefined || customer.email === 'bikersclan.bd@gmail.com' ? 'N/A' : customer.email}
                            </td>
                            <td className=" w-[100px]">

                                <NavLink
                                    to={`/customers/update/${customer.phone}`}
                                    className="mr-3 [&>svg]:text-yellow-400"
                                >
                                    <EditOutlinedIcon />
                                </NavLink>

                                <button onClick={() => handleDelete(customer._id)} className="[&>svg]:text-red-400"><DeleteOutlineOutlinedIcon /></button>
                            </td>
                        </tr>
                    ))}
                </tbody>
            ...
            <div className="mt-5">
                ...
                <button onClick={() => setCurrentPage(currentPage + 1)} disabled={endIndex >= filteredCustomers.length} className="ml-2 text-gray-600 [&>*]:!text-[16px]" >
